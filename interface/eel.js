eel = {
  ...
  _guid: ([1e7] + -1e3 + -4e3 + -8e3 + -1e11).replace(/[018]/g, (c) => (c ^ (crypto.getRandomValues(new Uint8Array(1))[0] & (15 >> (c / 4)))).toString(16)),
        ...
        if (message.hasOwnProperty("call")) {
          ...
          if (message.name in eel._exposed_functions) {
            try {
              ...
              debugger;
              eel._websocket.send(eel._toJSON({ return: message.call, status: "error", error: err.message, stack: err.stack }));
            }
          }
